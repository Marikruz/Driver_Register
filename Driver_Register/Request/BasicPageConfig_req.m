...
@implementation BasicPageConfig_req

-(NSURL *)url {
    if (_url == nil) {
        _url = [[NSURL alloc]initWithString:@"https://mis.didiglobal.com/gulfstream/deadpool/register/getBasicPageConfig"];
    }
    return _url;
}
...
- (void)dataTask {
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:self.mutablerequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (data) {
            self.path = [BasicPageConfig_req getPath];
            //保存数据
...
+ (NSString *)getPath {
    ...
    NSString *path =[documentsDirectory stringByAppendingPathComponent:@"basicpageconfig.json"];
    return path;
}
